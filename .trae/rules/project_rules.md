...
- **组件规范**：
  - **组件库优先**：所有 UI（布局、卡片、弹窗、提示消息、表单、导航等）必须优先使用 **Element Plus** 现有组件（本项目使用 `element-plus ^2.14.3`），仅当组件库确实没有匹配的组件时才允许自定义实现，且需在代码注释中说明原因
...
- **开发规则与约束**（SQLite 迁移、单实例锁、打包规范等）维护在根目录 `CLAUDE.md`，必须遵守。
- **各功能模块的设计决策与实现细节**维护在 `docs/design-decisions.md`，CLAUDE.md 中按条件引用；原始设计方案在 `docs/design/`。
- 遇到规则变更或重要修复时同步更新对应文件。
